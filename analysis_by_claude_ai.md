...
## Manually Updated on 2024.01.23
...
<details><summary> <b>2024-01-18 </b> Exposing Lip-syncing Deepfakes from Mouth Inconsistencies (Soumyya Kanti Datta et.al.)  <a href="http://arxiv.org/pdf/2401.10113.pdf">PDF</a> </summary>  <p>  Based on my review, here is a summary of the key elements of the paper:

1. The primary research objective is to develop a novel approach called LIPINC for detecting lip-syncing deepfake videos by identifying temporal inconsistencies in the mouth region. 

2. The hypothesis is that spatial-temporal inconsistencies in aspects like mouth shape, coloration, dental structure within adjacent frames as well as non-adjacent frames with similar poses can effectively distinguish between real and fake lip-sync videos.

3. The methodology employs facial landmark detection to extract mouth regions, followed by a module to retrieve adjacent and non-adjacent frames with similar mouth openness. These are input to a network that encodes color and structural mouth features to learn inconsistency patterns, classified by a binary classifier.  

4. Key results show the model outperforms state-of-the-art methods for in-domain testing on the FakeAVCeleb dataset with over 94% accuracy and AUC. It also demonstrates strong cross-domain generalization on other datasets. 

5. The authors interpret the findings to validate that spatial-temporal mouth inconsistencies can reliably detect lip-sync deepfakes, even on unseen datasets.

6. The main conclusion is that the proposed LIPINC approach leveraging local and global mouth inconsistencies provides an effective solution for identifying lip-sync deepfakes.  

7. Limitations like inability to handle videos without visible lips or very short videos are mentioned.

8. Future work suggested includes extending the approach to other partial manipulation detection and creating larger datasets. </p>  </details> 
